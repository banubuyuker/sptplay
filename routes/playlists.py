...
async def create_playlist(
    request: CreatePlaylistRequest, authorization: str = Header(...)
):
    """
    ...
    try:
        playlist = await client.create_playlist(
            name=request.name, description=request.description, public=request.public
        )
...
async def update_playlist(
    playlist_id: str, request: UpdatePlaylistRequest, authorization: str = Header(...)
):
    """
...
async def add_tracks(
    playlist_id: str, request: AddTracksRequest, authorization: str = Header(...)
):
    """
...
async def find_duplicates(playlist_id: str, authorization: str = Header(...)):
    ...
    try:
        ...
        for key, items in name_artist_counts.items():
            ...
            if len(unique_ids) > 1:
                similar_tracks.append({"track_key": key, "versions": items})
...
async def remove_duplicates(
    ...
    try:
        ...
        for idx, item in enumerate(tracks):
            track = item.get("track")
            if track and track.get("id"):
                track_list.append({"idx": idx, "id": track["id"], "uri": track["uri"]})
...
async def merge_playlists(
    request: MergePlaylistsRequest, authorization: str = Header(...)
):
    """
    ...
    try:
        ...
        new_playlist = await client.create_playlist(
            ...
            description=request.description
            or f"Merged from {len(request.source_playlist_ids)} playlists",
        )
...
async def restore_playlist(
    ...
    try:
        # Create new playlist
        new_playlist = await client.create_playlist(
            name=name, description="Restored from backup"
        )
